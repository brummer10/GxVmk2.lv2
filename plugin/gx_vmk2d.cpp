...
void Gx_vmk2d_::init_dsp_(uint32_t rate)
{
  ...
  driver->set_samplerate(rate, driver); 
  for(uint32_t i=0; i<MBR_COUNT; i++) {
        mbr[i] = mbrs_model[i]();
        mbr[i]->set_samplerate(rate, mbr[i]);
    }
  for(uint32_t i=0; i<VIBEREV_COUNT; i++) {
        viberev[i] = viberevs_model[i]();
        viberev[i]->set_samplerate(rate, viberev[i]);
    }
}
...
void Gx_vmk2d_::run_dsp_(uint32_t n_samples)
{
  ...
  mbr[mbr_model_]->mono_audio(static_cast<int>(n_samples), output, output, mbr[mbr_model_]);

  // run selected viberev model
  if (rev_model_) {
    viberev[rev_model_-1]->mono_audio(static_cast<int>(n_samples), output, output, viberev[rev_model_-1]);
  }
  // driver/mixer stage
  driver->mono_audio(static_cast<int>(n_samples), output, output, driver);
}
