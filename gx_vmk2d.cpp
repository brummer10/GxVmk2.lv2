...
Gx_vmk2d_::~Gx_vmk2d_()
{
  ...
  if (vmk2d->activate_plugin !=0)
    vmk2d->activate_plugin(false, vmk2d);
  // delete DSP class
  vmk2d->delete_instance(vmk2d);
};
...
{
  vmk2d->set_samplerate(rate, vmk2d); // init the DSP class
}
...
void Gx_vmk2d_::activate_f()
{
  ...
  if (vmk2d->activate_plugin !=0)
    vmk2d->activate_plugin(true, vmk2d);
}
...
{
  vmk2d->mono_audio(static_cast<int>(n_samples), input, output, vmk2d);
}
...
void Gx_vmk2d_::connect_all__ports(uint32_t port, void* data)
{
  ...
  vmk2d->connect_ports(port,  data, vmk2d);
}
