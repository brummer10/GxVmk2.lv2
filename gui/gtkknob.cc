...
#define GTK_VMK_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_VMK_KNOB, GtkVmkKnobPrivate))

G_DEFINE_TYPE(GtkVmkKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkVmkKnobPrivate GtkVmkKnobPrivate;

struct _GtkVmkKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	/** check resize **/
	int grow;
 	GtkVmkKnobPrivate *priv = GTK_VMK_KNOB_GET_PRIVATE(widget);
	...
	double lengh_x = (knobx+radius+pointer_off/2) - radius/ 1.01 * sin(angle);
	double lengh_y = (knoby+radius+pointer_off/2) + radius/ 1.01 * cos(angle);
	//double radius1 = min(knob_x, knob_y) / 2 ; // center
	double radius_x = (knobx+radius+pointer_off/2) - radius/ 1.05 * sin(angle);
	double radius_y = (knoby+radius+pointer_off/2) + radius/ 1.05 * cos(angle);
	...
	pat =
		cairo_pattern_create_radial (knobx1+arc_offset-knob_x/8,knoby1+arc_offset-knob_x/8, 1,knobx1+arc_offset,knoby1+arc_offset,knob_x/2.1 );
	 if (priv->model == 1 || priv->model == 3) {
        if (knobstate > 0.6) {
	...
	cairo_set_line_width(cr,min(5, max(4,knob_x/30)));
    cairo_set_source_rgb (cr,0.1,0.1,0.1);
	cairo_stroke_preserve(cr);
	cairo_move_to(cr, radius_x, radius_y);
	cairo_line_to(cr,lengh_x,lengh_y);
	cairo_set_line_width(cr,min(4, max(3,knob_x/30)));
...
{
	g_assert(GTK_IS_VMK_KNOB(widget));
	GtkVmkKnob *knob = GTK_VMK_KNOB(widget);
	GtkVmkKnobPrivate *priv = GTK_VMK_KNOB_GET_PRIVATE(knob);
...
/****************************************************************
 ** init the GtkVmkKnobClass
 */

static void gtk_knob_class_init (GtkVmkKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkVmkKnobPrivate));
}
...
static void gtk_knob_init (GtkVmkKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_VMK_KNOB, NULL ));
