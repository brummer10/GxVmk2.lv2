...
G_DEFINE_TYPE(GxVmkPaintBox, gx_paint_box, GTK_TYPE_BOX)

static void gx_paint_box_class_init (GxVmkPaintBoxClass *klass)
{
...
static void set_paint_func(GxVmkPaintBox *paint_box, const gchar *paint_func)
{
	gchar *spf;
...
static void gx_paint_box_style_set(GtkWidget *widget, GtkStyle  *previous_style)
{
	GxVmkPaintBox *paint_box = GX_VMK_PAINT_BOX(widget);
	set_paint_func(paint_box, paint_box->paint_func);
}


static void gx_paint_box_init (GxVmkPaintBox *paint_box)
{
...
static void gx_paint_box_destroy(GtkObject *object)
{
	GxVmkPaintBox *paint_box = GX_VMK_PAINT_BOX(object);
	...
	GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_PAINT_FUNC:
		g_value_set_string(value, GX_VMK_PAINT_BOX(object)->paint_func);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
static void pedal_expose(GtkWidget *wi, GdkEventExpose *ev)
{
	...
	cairo_t *cr;
	GxVmkPaintBox *paintbox = GX_VMK_PAINT_BOX(wi);
